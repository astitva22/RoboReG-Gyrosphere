...
body = pb.loadURDF("/home/astitva/Projects/robotics/Research-Gyrosphere/kinematicsSim/outershell.urdf",[0,0,1.01])
'''
cv2.namedWindow('Controls')
cv2.createTrackbar('fwd','Controls',0,10,callBack)
cv2.createTrackbar('side','Controls',0,10,callBack)
cv2.createTrackbar('vertical','Controls',0,10,callBack)
'''  
fwd = 0
side = 0   
motor_torque = 50  
while(True):
    ...
    time.sleep(1./240.)
    '''
    desired_ang_vel = [[cv2.getTrackbarPos('side','Controls')],
                       [cv2.getTrackbarPos('fwd','Controls')],
                       [cv2.getTrackbarPos('vertical','Controls')]]
    cv2.waitKey(1)
    '''
